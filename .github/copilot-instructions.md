...
## Useful commands (PowerShell / Windows):
  - Build: `dotnet build` from solution root
  - Test: `dotnet test --filter Category!=Integration` to run fast tests
  - Run: `dotnet run --project src/YourProject.Api` for local API
